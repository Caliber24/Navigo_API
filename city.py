...
def get_all_states():
    try:
        res = requests.get("https://iran-locations-api.vercel.app/api/v1/fa/states", timeout=10)
...
def get_cities_for_state(state_name):
    try:
        # URL-encode کردن نام استان برای جلوگیری از مشکلات کاراکترهای فارسی
        ...
        if res.status_code == 200:
            ...
            if isinstance(data, list) and len(data) > 0 and "cities" in data[0]:
                ...
                # اگر کلید "cities" وجود نداشته باشد، فرض می‌کنیم لیست مستقیم شهرها است
                return data
            print(f"Error: Cities response for {state_name} does not match expected format: {data}")
            return []
        else:
            print(f"Error: Cities API for {state_name} returned status code {res.status_code}")
            return []
    except requests.RequestException as e:
    ...
    for state in states:
        ...
        for city in cities:
            ...
            if city_name:
                destination = {
                    "name": city_name,
                    "province": state_name  # اطمینان از تخصیص صحیح استان
                }
                # افزودن مختصات اگر وجود داشته باشند
                ...
                # افزودن سایر فیلدهای اضافی
                for key, value in city.items():
                    if key not in ["name", "latitude", "longitude"] and value is not None:
                        destination[key] = value
                destinations.append(destination)
            else:
                print(f"Warning: City name is missing for {state_name}, skipping: {city}")
